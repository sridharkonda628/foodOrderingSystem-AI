...
export const AISearchHero: React.FC<AISearchHeroProps> = ({ onResultsFound }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-orange-950 text-white p-6 sm:p-10 shadow-2xl border border-slate-700/50 my-6">
      ...
      <div className="relative z-10 max-w-3xl mx-auto text-center">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-orange-500/20 border border-orange-500/30 text-orange-300 text-xs font-bold uppercase tracking-wider mb-4 animate-pulse">
        ...
        >
          ...
          <input
            ...
            className="w-full pl-12 pr-28 py-4 bg-white/10 hover:bg-white/15 focus:bg-white/20 text-white placeholder-slate-400 rounded-2xl border border-white/20 focus:outline-hidden focus:ring-2 focus:ring-orange-500 backdrop-blur-md text-sm sm:text-base transition"
          />
          <button
            type="submit"
            disabled={isLoading || !query.trim()}
            className="absolute right-2 px-5 py-2.5 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 disabled:opacity-50 text-white font-bold rounded-xl text-sm flex items-center gap-1.5 shadow-md transition cursor-pointer"
          >
            {isLoading ? (
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 text-xs">
          ...
          {EXAMPLE_QUERIES.map((ex, idx) => (
            <button
              ...
              className="bg-white/10 hover:bg-orange-500/30 border border-white/15 hover:border-orange-400/50 text-slate-200 hover:text-white px-3 py-1.5 rounded-full transition cursor-pointer"
            >
              {ex}
            </button>
          ))}
        ...
        <div className="relative z-10 mt-8 pt-8 border-t border-white/15">
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-white/15 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-white/10">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-orange-500/30 flex items-center justify-center text-orange-400">
                  <Zap className="w-4 h-4" />
                </div>
                <div>
                  <span className="text-xs text-slate-400 block uppercase font-bold tracking-wider">AI Search Intent</span>
                  <span className="text-sm font-bold text-white">"{searchData.query}"</span>
                </div>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <span className="bg-emerald-500/20 text-emerald-300 px-2.5 py-1 rounded-full border border-emerald-500/30 font-medium">
                  ⚡ {searchData.execution_time_ms}ms
                </span>
                <span className="bg-blue-500/20 text-blue-300 px-2.5 py-1 rounded-full border border-blue-500/30 font-medium uppercase text-[10px]">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3 text-xs">
              <span className="text-slate-300 font-semibold">Detected Constraints:</span>
              ...
              {searchData.detected_intent.max_price !== null && (
                <span className="bg-amber-500/30 text-amber-300 border border-amber-500/40 px-2.5 py-0.5 rounded-full font-bold">
                  💰 Max Price: ₹{searchData.detected_intent.max_price}
                </span>
              )}

              {searchData.detected_intent.preferred_tags?.map((t, idx) => (
                <span key={idx} className="bg-purple-500/30 text-purple-300 border border-purple-500/40 px-2.5 py-0.5 rounded-full font-medium">
                  🏷️ {t}
                </span>
              ))}
            </div>
          ...
          {searchData.items.length === 0 ? (
            <div className="text-center py-10 bg-white/5 rounded-2xl border border-white/10">
              <p className="text-slate-300 font-semibold mb-1">No dishes matched your exact criteria.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-slate-900">
              {searchData.items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="bg-white rounded-2xl p-4 shadow-lg border border-slate-100 flex flex-col justify-between hover:shadow-xl transition"
                  >
                    <div>
                      <div className="flex justify-between items-start gap-2 mb-2">
                        ...
                        <span className="bg-gradient-to-r from-orange-500 to-amber-500 text-white font-black text-[11px] px-2 py-0.5 rounded-full shadow-xs">
                          {relevancePct}% Match
                        </span>
                      </div>

                      <h4 className="font-extrabold text-base text-slate-900 leading-snug">{item.name}</h4>
                      ...
                      <div className="mt-2.5 bg-orange-50 border border-orange-200/70 rounded-xl p-2 text-[11px] text-orange-900 font-medium flex items-start gap-1.5">
                        ...
                        <span>{item.match_explanation}</span>
                      </div>
                    </div>
